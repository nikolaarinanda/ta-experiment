...
class CyberbullyingCommentDataset(Dataset):
    def __init__(
        self,
        file_path="dataset/dataset_cyberbullying.xlsx",
        tokenizer_name="indobenchmark/indobert-base-p1",
        folds_file="cyberbullying-datareader-simple-folds.json",
        random_state=2002,
        split="train",
        fold=0,
        ...
        super(CyberbullyingCommentDataset, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if sentiment == -1:
            label = 0  # Label PyTorch untuk 'cyberbullying'
        elif sentiment == 1:
            label = 1  # Label PyTorch untuk 'non-cyberbullying'
        # else:
        #     original_comment = self.df.iloc[idx]['comment']
        #     raise ValueError(f"Sentiment tidak dikenali: {sentiment} (index: {idx})\nKomentar: {original_comment}")
    ...
    def preprocess_text(self, text):
        ...
        text = re.sub(r'(.)\1{2,}', r'\1', text)              # reduksi karakter berulang (contoh: "kampreeeet" -> "kampret")

        # Tokenisasi
        tokens = word_tokenize(text)

        # Ganti frasa yang cocok (replace_phrases)
        # if hasattr(self, 'augmentasi_data') and "replace_phrases" in self.augmentasi_data:
        #     for i in range(len(tokens)):
        #         token = tokens[i]
        #         if token in self.augmentasi_data["replace_phrases"]:
        #             tokens[i] = random.choice(self.augmentasi_data["replace_phrases"][token])
    ...
    def load_data(self):
        ...
        self.df = self.df.dropna(subset=['comment', 'sentiment'])
...
if __name__ == "__main__":
    ...
    train_dataset = CyberbullyingCommentDataset(fold=0, split="train", augment_prob=args.augment_prob)
    val_dataset = CyberbullyingCommentDataset(fold=0, split="val", augment_prob=0.0)
